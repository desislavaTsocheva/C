...
void readFromFp(const char* binaryName, FILE* fp) {
    ...
    rewind(fp); // <==> feseek(fp, 0 SEEK_SET) + if
    ...
    const char* txt = "C:\\Users\\viole\\OneDrive\\Работен плот\\C\\File_2\\secondproblem.txt";
    bubbleSort(arr, len);
...
{
    FILE* fp;
    const char* binaryName = "C:\\Users\\viole\\OneDrive\\Работен плот\\C\\File_2\\secondproblem.bin";
